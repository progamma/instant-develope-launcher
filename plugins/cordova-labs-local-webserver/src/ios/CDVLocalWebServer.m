...
static NSUInteger staticPort = -1;
static NSString* staticToken = @"";
...
- (void)getPort:(CDVInvokedUrlCommand*)command
{
    ...
    result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsInt:staticPort];
...
{
    CDVPluginResult* result = nil;
    result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:staticToken];
    [self.commandDelegate sendPluginResult:result callbackId:[command callbackId]];
}
...
-  (void) addFileSystemHandlers:(NSString*)authToken
{
    ...
    SEL sel = NSSelectorFromString(@"setUrlTransformerStocazzo:");
...
- (void) addFileSystemHandler:(GCDWebServerAsyncProcessBlock)processRequestForResponseBlock basePath:(NSString*)basePath authToken:(NSString*)authToken cacheAge:(NSUInteger)cacheAge
{
    ...
    GCDWebServerAsyncProcessBlock asyncProcessBlock = ^void (GCDWebServerRequest* request, GCDWebServerCompletionBlock complete) {
        ...
        BOOL hasReferer = (referer && [referer containsString:authToken]);
        if (!hasToken && !hasCookie && !hasReferer) {
            complete([GCDWebServerErrorResponse responseWithClientError:kGCDWebServerHTTPStatusCode_Forbidden message:@"FORBIDDEN"]);
        }
        ...
        processRequestForResponseBlock(request, ^void(GCDWebServerResponse* response){
            ...
            if ((hasToken || hasReferer) && !hasCookie) {
                //set cookie
                NSString *coo = [authToken stringByAppendingFormat:@"; Path=/"];
                [response setValue:coo forAdditionalHeader:@"Set-Cookie"];
            }
            complete(response);
        });
    };
...
- (void) addLocalFileSystemHandler:(NSString*)authToken
{
    ...
    GCDWebServerAsyncProcessBlock processRequestBlock = ^void (GCDWebServerRequest* request, GCDWebServerCompletionBlock complete) {
        ...
        //NSString* filePath = [request.path substringFromIndex:basePath.length-1];
        //NSArray *paths = NSSearchPathForDirectoriesInDomains (NSDocumentDirectory, NSUserDomainMask, YES);
        //filePath = [[paths objectAtIndex:0] stringByAppendingString:filePath];
